...
const JuniorCode: React.FC<JuniorCodeProps> = ({ userData }) => {
  const [activeView, setActiveView] = useState<'menu' | 'maze' | 'flappy' | 'breakout' | 'dino' | 'battle' | 'logic' | 'decoder' | 'factory' | 'cargo' | 'fundamentals' | 'scratch' | 'maze-battle' | 'pixel-art'>('menu');
      ...
      <div className="max-w-6xl mx-auto space-y-12 pb-20">
        ...
        <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {/* Pixel Art Card */}
                <button 
                    onClick={() => setActiveView('pixel-art')}
                    className="group relative h-80 rounded-[2rem] overflow-hidden border border-slate-800 transition-all hover:border-pink-500 hover:shadow-2xl hover:shadow-pink-500/20 text-left bg-slate-900"
                >
                     <div className="absolute inset-0 bg-slate-900"/>
                     <div className="absolute inset-0 bg-gradient-to-br from-pink-600/20 to-rose-600/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500"/>
                    ...
                    <div className="absolute inset-0 flex flex-col p-8 z-10">
                        <div className="flex justify-between items-start mb-auto">
                            <div className="w-12 h-12 bg-pink-500 rounded-2xl flex items-center justify-center shadow-lg shadow-pink-500/30 group-hover:scale-110 transition-transform duration-300">
                                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                            </div>
                            <span className="bg-slate-950/50 backdrop-blur border border-slate-700 text-slate-300 text-[10px] font-black uppercase px-3 py-1 rounded-full">Art Studio</span>
                        </div>

                        <div className="space-y-4">
                            <h3 className="text-4xl font-black text-white italic tracking-tighter uppercase group-hover:text-pink-400 transition-colors">Pixel<br/>Art</h3>
                            <p className="text-slate-400 font-medium leading-relaxed group-hover:text-slate-300 max-w-xs">Create digital masterpieces! Recreate shapes block by block.</p>
                            
                            <div className="flex items-center space-x-2 text-pink-400 font-bold uppercase text-xs tracking-wider group-hover:translate-x-2 transition-transform">
