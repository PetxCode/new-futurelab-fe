...
// ─── Animated Counter ────────────────────────────────────────────────────────
const AnimatedNumber: React.FC<{ target: string; label: string }> = ({
  target,
  label,
}) => {
  ...
  return (
    <div ref={ref} className="text-center">
      <p className="text-4xl sm:text-5xl font-black text-white tracking-tighter tabular-nums">
        {displayed}
      </p>
      <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] mt-2">
        {label}
      </p>
    </div>
  );
...
const FeatureCard: React.FC<{
  ...
  return (
    ...
    >
      {/* top-line accent */}
      <div
        className={`absolute top-0 left-0 right-0 h-px ${glowColor} opacity-60`}
      />
      {/* hover glow blob */}
      <div
        className={`absolute -top-8 -right-8 w-32 h-32 rounded-full ${iconBg} blur-2xl opacity-0 group-hover:opacity-40 transition-opacity duration-500`}
      />

      <div
        className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${iconBg} bg-opacity-20 border border-white/5`}
      >
        {icon}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center shrink-0">
        <div
          className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg text-white shadow-lg ${c.badge}`}
        >
          {number}
        </div>
        {number < 4 && (
          <div className="w-px flex-1 min-h-[3rem] bg-slate-800 mt-1" />
        )}
      </div>
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          {tools.map((t) => (
            <span
              key={t}
              className="px-3 py-1 bg-slate-800/80 border border-slate-700/60 text-slate-300 text-[10px] font-bold uppercase tracking-wider rounded-lg"
            >
              {t}
            </span>
          ))}
      ...
      <div className="px-6 pb-4 flex-1">
        <p className="text-slate-400 text-sm leading-relaxed line-clamp-2 mb-4 italic">
          ...
          {inst.instructorProfile?.bio ||
            "Passionate about empowering the next generation of Nigerian tech talent."}
          "
        </p>
        <div className="flex flex-wrap gap-1.5">
          {(inst.instructorProfile?.specialties?.length
            ...
            .map((s: string) => (
              <span
                key={s}
                className="px-2.5 py-1 bg-slate-800/80 text-slate-300 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-slate-700/60"
              >
                {s}
              </span>
            ))}
...
const testimonials = [
  {
    quote:
      "My daughter went from zero to building her own game in 3 months. FutureLab's structured ladder approach is unlike anything else out there.",
    name: "Mrs. Adeyemi",
    role: "Parent · Level 2 Student",
    avatar: "A",
    from: "#6366f1",
    to: "#4338ca",
  },
  {
    quote:
      "I finally understand machine learning and I'm only 14. My FutureLab instructor made it feel like a superpower — not just theory.",
    name: "Tunde O.",
    role: "Level 4 · Master Architect Student",
    avatar: "T",
    from: "#10b981",
    to: "#059669",
  },
  {
    quote:
      "We partnered FutureLab for our after-school program. The kids are more engaged than ever — competing to build the best projects every week.",
    name: "Mr. Chukwuemeka",
    role: "Academic Director · Zion Anglican School",
    avatar: "C",
    from: "#f59e0b",
    to: "#d97706",
  },
...
const ProcessStep: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="text-[80px] font-black leading-none text-slate-800/60 select-none"
        style={{ fontFamily: '"Outfit", sans-serif' }}
      >
        0{n}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-slate-100 overflow-x-hidden selection:bg-indigo-500/30">
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {[
              ...
              <a
                ...
                style={{
                  transitionDelay: navDelay(i),
                  opacity: heroIn ? 1 : 0,
                  transform: heroIn ? "none" : "translateY(-6px)",
                  transition:
          ...
          >
            <button
              onClick={onLogin}
              className="px-5 py-2.5 text-sm font-black text-slate-300 hover:text-white hover:bg-white/5 rounded-xl transition-all duration-200"
            >
              Log In
            </button>
            <button
          ...
          <div className="md:hidden absolute top-full left-0 right-0 bg-[#0d1220]/98 backdrop-blur-xl border-b border-white/[0.06] px-6 py-6 space-y-3">
            {[
              ...
              <a
                ...
                className="block py-2 text-slate-200 font-bold hover:text-white transition-colors"
              >
                {l}
              </a>
            ))}
            <div className="flex gap-3 pt-3 border-t border-white/5">
              <button
                ...
                className="flex-1 py-3 text-sm font-black text-white rounded-xl bg-white/[0.08] border border-white/[0.1]"
              >
                Log In
              </button>
              <button
                ...
                className="flex-1 py-3 text-sm font-black text-white rounded-xl bg-indigo-600 shadow-lg shadow-indigo-600/30"
              >
                Get Started
              </button>
            </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 pt-24 pb-20">
        ...
        {/* Corner accent */}
        <div
          className="absolute top-0 right-0 w-[420px] h-[420px] -z-10"
          style={{
            background:
              "radial-gradient(circle at top right, rgba(52,211,153,0.07) 0%, transparent 65%)",
          }}
        />
        <div
          className="absolute bottom-0 left-0 w-[300px] h-[300px] -z-10"
          style={{
            background:
              "radial-gradient(circle at bottom left, rgba(99,102,241,0.08) 0%, transparent 65%)",
          }}
        />
        ...
        {[
          ...
          <div
            key={i}
            className="absolute font-mono font-bold text-emerald-400/[0.07] pointer-events-none select-none whitespace-nowrap"
            style={s}
          >
            {code}
          </div>
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div
            ...
            style={{
              ...
              transition:
                "opacity 0.65s ease 0.05s, transform 0.65s ease 0.05s",
            }}
          >
          ...
          >
            ...
            <span
              ...
              style={{
                background:
                  "linear-gradient(135deg, #818cf8 0%, #38bdf8 45%, #34d399 100%)",
                WebkitBackgroundClip: "text",
                WebkitTextFillColor: "transparent",
                backgroundClip: "text",
              }}
          ...
          <div className="text-indigo-300 text-[11px] font-black uppercase tracking-[0.65em] -mt-4 mb-8">
            Master AI, Robotic and Coding for Young Learners.
          </div>
          {/* Sub */}
          <p
          ...
          >
            From block-coding at age 5 to machine learning at 14. FutureLab's
            structured Coder's Ladder guides Nigeria's young minds from curious
            beginners to confident builders.
          </p>
          ...
          >
            {[
              { target: "3.5k+", label: "Students Taught" },
              { target: "2.5M", label: "Hours of Code" },
              { target: "16+", label: "Camps Completed" },
              { target: "12+", label: "Schools Partnered" },
            ].map((s) => (
              <AnimatedNumber key={s.label} {...s} />
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            <SectionHeading>
              Simple Steps to
              <br />
              <Accent gradient="linear-gradient(135deg,#34d399,#38bdf8)">
                Greatness.
              </Accent>
            </SectionHeading>
            <p className="text-slate-400 text-lg font-medium max-w-xl mx-auto">
              Getting started is effortless. Learning is structured. Progress is
              inevitable.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            ...
            {[
              ...
              },
              {
                ...
                desc: "Take a short placement quiz — or start at Level 1. There's no wrong answer.",
              },
              {
                n: 3,
                title: "Learn & Build",
                desc: "Weekly live classes, self-paced modules, and hands-on project challenges.",
              },
              {
                n: 4,
                title: "Level Up & Ship",
                desc: "Complete projects, earn XP, and advance the Coder's Ladder to Master Architect.",
              },
            ].map(({ n, title, desc }) => (
              <ProcessStep
                key={n}
      ...
      <section id="programs" className="py-28 px-6 relative">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            <p className="text-slate-400 text-lg font-medium max-w-xl mx-auto">
              Every learning style, every schedule. Built for Nigeria's
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            {[
              ...
              <div
                key={f.label}
                className="flex items-center gap-2.5 px-5 py-3 rounded-2xl border border-slate-800 bg-slate-900/40 hover:border-slate-700 hover:bg-slate-800/40 transition-all cursor-default"
              >
      ...
      <section id="ladder" className="py-28 px-6 relative">
        <div
          className="absolute top-0 right-0 w-[500px] h-[500px] -z-10"
          style={{
            background:
              "radial-gradient(circle at top right, rgba(16,185,129,0.05) 0%, transparent 60%)",
          }}
        />
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at bottom left, rgba(99,102,241,0.06) 0%, transparent 60%)",
          }}
        />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
            ...
            <div className="lg:sticky lg:top-28">
              ...
              <p className="text-slate-400 text-lg font-medium leading-relaxed mb-6">
                Designed by CS experts. Inspired by martial arts belt systems.
                Each level unlocks new superpowers — and every child starts
                exactly where they should.
              </p>
              <p className="text-slate-500 text-sm font-medium leading-relaxed mb-10">
                From dragging blocks in ScratchJr to deploying machine learning
                models — our proprietary 4-level curriculum covers the complete
                journey of a modern software engineer.
              </p>

              <div className="flex flex-wrap gap-3 mb-12">
                <button
                  onClick={onStart}
                  className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-lg shadow-indigo-600/20 transition-all hover:scale-105 active:scale-95 text-sm"
                >
                  Find Your Level →
                </button>
                <button
                  ...
                  className="px-8 py-4 bg-white/[0.05] hover:bg-white/[0.08] text-white font-black rounded-2xl border border-white/[0.1] transition-all text-sm"
                >
                  Meet Trainers
                </button>
              </div>
                ...
                {[
                  {
                    emoji: "👾",
                    title: "Game Design",
                    sub: "Build interactive worlds",
                  },
                  {
                    emoji: "🤖",
                    title: "Robotics",
                    sub: "Hardware meets code",
                  },
                  {
                    emoji: "🌐",
                    title: "Web Mastery",
                  ...
                  },
                  {
                    emoji: "🧠",
                    title: "AI & Data",
                    sub: "Python & Machine Learning",
                  },
                ].map((s) => (
                  <div
                    key={s.title}
                    className="p-4 rounded-2xl border border-slate-800 bg-slate-900/50 flex items-center gap-3 hover:border-slate-700 hover:bg-slate-800/40 transition-all group cursor-default"
                  >
        ...
        <section id="trainers" className="py-28 px-6 relative">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-20">
              ...
              <p className="text-slate-400 text-lg font-medium max-w-xl mx-auto">
                Hand-picked, certified instructors. Each one a practitioner
                first, educator second — so your child learns from people
                actively building with the tools they teach.
              </p>
            </div>
                  ...
                  onEngage={() => {
                    localStorage.setItem(
                      ...
                      JSON.stringify({
                        ...
                        rate: inst.instructorProfile?.monthlyRate || 20000,
                      }),
                    );
                    onStart();
                  }}
            ...
            {instructors.length > 6 && (
              <div className="mt-12 text-center">
                <button
                  ...
                  className="inline-flex items-center gap-3 px-10 py-5 bg-white/[0.05] hover:bg-white/[0.08] text-white font-black rounded-2xl border border-white/[0.1] hover:border-white/[0.2] transition-all text-sm"
                >
            ...
            {testimonials.map((t, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Quote mark */}
                  <svg
                    className="w-9 h-9 text-slate-700 mb-6 fill-current"
                    viewBox="0 0 24 24"
                  >
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-16 gap-8">
              ...
              <p className="text-slate-400 font-medium max-w-xs leading-relaxed text-sm">
                Articles, tutorials, and announcements from our instructors and
                the FutureLab team.
              </p>
            </div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg,#1e1b4b 0%,#1a3a5c 50%,#064e3b 100%)",
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle at 25% 50%, rgba(99,102,241,0.18) 0%, transparent 55%), radial-gradient(circle at 75% 50%, rgba(16,185,129,0.12) 0%, transparent 55%)",
              }}
            />
            {/* decorative circle */}
            ...
            <div className="relative">
              ...
              <p className="text-slate-300 text-lg font-medium max-w-lg mx-auto mb-10 leading-relaxed">
                Join thousands of Nigerian students building real skills, real
                projects, and a real future in technology.
              </p>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4">
              {[
                ...
                <a
                  ...
                  href="#"
                  aria-label={s.label}
                  className="w-9 h-9 rounded-xl bg-white/[0.05] border border-white/[0.08] flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/[0.1] transition-all"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
          ...
          <div className="pt-8 border-t border-white/[0.05] text-center">
            <p className="text-slate-600 text-sm font-medium">
              © {new Date().getFullYear()} FutureLab: A NEXT Project. All rights
              reserved.
            </p>
          </div>
        </div>
      ...
      {selectedTrainer && (
        ...
        >
          <div
            className="bg-[#0d1220] border border-slate-700 w-full max-w-2xl max-h-[88vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="relative p-8 flex items-start gap-6 border-b border-slate-800">
              <button
                onClick={() => setSelectedTrainer(null)}
                className="absolute top-5 right-5 p-2 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-xl transition-all"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
              ...
              <img
                src={
                  ...
                  `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedTrainer.fullName)}&background=4338ca&color=fff&size=200&bold=true`
                }
              ...
              <div>
                ...
                <p className="text-indigo-400 text-xs font-black uppercase tracking-widest mt-1">
                  {selectedTrainer.instructorProfile?.yearsExperience || 1}+
                  Years Experience
                </p>
                <div className="flex flex-wrap gap-2 mt-3">
                  {(
                    selectedTrainer.instructorProfile?.specialties || [
                      "Python",
                      "Robotics",
                    ]
                  ).map((s: string) => (
                    <span
                      key={s}
                      className="px-3 py-1 bg-slate-800 text-slate-300 rounded-lg text-xs font-bold border border-slate-700"
                    >
                      {s}
                    </span>
                  ))}
            ...
            <div className="flex-1 overflow-y-auto p-8">
              ...
              <p className="text-slate-300 leading-relaxed">
                {selectedTrainer.instructorProfile?.bio ||
                  "Expert educator focused on empowering students through hands-on, project-based learning."}
              </p>
            </div>
            <div className="p-8 border-t border-slate-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-white text-2xl font-black">
                  ₦
                  {(
                    selectedTrainer.instructorProfile?.monthlyRate || 20000
                  ).toLocaleString()}
                </p>
              </div>
              <button
                onClick={() => {
                  localStorage.setItem(
                    ...
                    JSON.stringify({
                      ...
                      rate: selectedTrainer.instructorProfile?.monthlyRate,
                    }),
                  );
                  onStart();
                }}
