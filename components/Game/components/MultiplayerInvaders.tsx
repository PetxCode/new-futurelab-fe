...
export default function MultiplayerInvaders({
  onExit,
}: MultiplayerInvadersProps) {
  // ── Connection / lobby ──────────────────────────────────────────────
      ...
      setActiveInvaders((prev) => {
        ...
        const updated = prev
          .map((inv) => {
            ...
            const newY =
              inv.y + 0.0005 * currentLevel.speedMultiplier * serverSpeed * 16;
          ...
          setLives((l) => {
            ...
            if (next <= 0) {
              ...
              if (socketRef.current && roomIdRef.current) {
                socketRef.current.emit("invaders_game_over", {
  ...
  const handleSetSpeed = (val: number) => {
    setLobbySpeed(val);
    socket?.emit("invaders_set_speed", {
          ...
          <div className="flex-1 flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">
                Last defender standing wins! Type the correct Tailwind class to
                destroy invaders.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-[#0a0e1a] border border-gray-700 rounded-xl text-white outline-none focus:border-cyan-500 transition-colors font-bold text-sm"
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex bg-[#0a0e1a] p-1 rounded-xl border border-gray-800">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${isCreating ? "bg-cyan-600 text-white shadow-lg" : "text-gray-400 hover:text-white"}`}
                >
                  Create Room
                </button>
                <button
            ...
            {!isCreating && (
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#0a0e1a] border border-gray-700 rounded-xl text-white outline-none focus:border-cyan-500 transition-colors font-mono font-bold text-sm tracking-widest uppercase text-center"
                />
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-black rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-transform transform active:scale-95 text-sm uppercase tracking-wider flex items-center justify-center gap-2"
            >
            ...
            <div className="grid grid-cols-4 gap-2 bg-[#0a0e1a] p-4 rounded-2xl border border-gray-800 overflow-y-auto max-h-[320px]">
              {CHARACTERS.map((c) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2.5 rounded-xl border-2 transition-all hover:scale-105 ${selectedChar?.id === c.id ? "border-cyan-500 bg-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.3)]" : "border-transparent bg-[#111827] hover:border-gray-700"}`}
                >
                  <span className="text-3xl">{c.emoji}</span>
                  <span className="text-[9px] uppercase font-bold tracking-wide mt-1 text-gray-400">
                    {c.name}
                  </span>
                </button>
              ))}
      ...
      <div className="w-full min-h-full bg-[#0a0e1a] flex flex-col items-center justify-center p-6 text-gray-200 overflow-auto">
        <div className="w-full max-w-2xl bg-[#111827] border border-cyan-500/20 rounded-3xl p-8 shadow-[0_0_60px_rgba(6,182,212,0.15)] flex flex-col gap-6 my-8">
          <div className="text-center">
            <span className="px-3 py-1 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 rounded-full text-xs font-black uppercase tracking-widest">
              Battle Lobby
            </span>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Share this code — last defender alive wins! 👾
            </p>
          </div>
          ...
          {isHost && (
            <div className="bg-[#0a0e1a] border border-gray-800 rounded-2xl p-4 flex flex-col gap-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-cyan-400">
                ⚡ Invader Speed (Host Setting)
              </h3>
              <div className="grid grid-cols-2 gap-2">
                {SPEED_PRESETS.map((p) => (
                  <button
                    key={p.value}
                    onClick={() => handleSetSpeed(p.value)}
                    className={`py-3 rounded-xl font-bold text-sm transition-all ${lobbySpeed === p.value ? "bg-cyan-600 text-white shadow-[0_0_15px_rgba(6,182,212,0.4)]" : "bg-[#111827] border border-gray-800 text-gray-400 hover:border-cyan-500/40 hover:text-cyan-300"}`}
                  >
                    {p.label}
                  </button>
                ))}
              ...
              <p className="text-[10px] text-gray-500 text-center">
                This speed applies to everyone in the room.
              </p>
            </div>
          )}
            ...
            <div className="bg-[#0a0e1a] border border-gray-800 rounded-2xl p-4 text-center">
              <p className="text-xs text-gray-400">
                ...
                <span className="font-bold text-cyan-400">
                  {SPEED_PRESETS.find((p) => p.value === lobbySpeed)?.label ??
                    `${lobbySpeed}x`}
                </span>
              </p>
            </div>
          ...
          <div className="bg-[#0a0e1a] border border-gray-800 rounded-2xl p-4 flex flex-col gap-2">
            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 border-b border-gray-800 pb-2">
              Defenders ({players.length})
            </h3>
            <div className="grid grid-cols-2 gap-3 mt-2">
              {players.map((p, i) => (
                <div
                  key={p.id || i}
                  className="flex items-center gap-3 bg-[#111827] p-3 rounded-xl border border-gray-800"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-500 font-bold uppercase">
                      {p.id === hostId ? "Host 👑" : "Defender"}
                    </p>
                  </div>
                  {p.id === socket?.id && (
                    <span className="px-2 py-0.5 bg-cyan-500/20 border border-cyan-500/40 text-cyan-400 rounded text-[9px] font-bold">
                      You
                    </span>
                  )}
                </div>
          ...
          <div className="flex flex-col gap-3">
            {isHost ? (
              <button
                ...
                className="w-full py-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-black rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.4)] transition transform active:scale-95 text-lg uppercase tracking-wider"
              >
                👾 Launch Invasion!
              </button>
            ) : (
      ...
      <div className="flex w-full h-full bg-[#0a0e1a] text-white font-sans overflow-hidden flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden relative">
          ...
          <div className="px-4 py-2 bg-[#111827] border-b border-cyan-500/30 flex justify-between items-center z-10 shrink-0">
            <div className="flex items-center gap-3">
              <h2 className="text-base font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500 uppercase tracking-tight">
                👾 Invaders
              </h2>
              ...
              <span className="text-[10px] font-black text-cyan-300 uppercase tracking-widest bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20">
                {SPEED_PRESETS.find((p) => p.value === serverSpeed)?.label ??
                  `${serverSpeed}x`}
              </span>
            </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <div className="flex gap-0.5 text-rose-500 text-xs">
                  {Array.from({ length: 10 }).map((_, i) => (
                    <span
                      key={i}
                      className={i < lives ? "opacity-100" : "opacity-10"}
                    >
                      ❤️
                    </span>
                  ))}
            ...
            {isGameOver && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-rose-950/80 backdrop-blur-md z-20">
                <div className="text-center p-8 bg-[#111827] rounded-3xl border-2 border-rose-500 shadow-[0_0_80px_rgba(225,29,72,0.4)]">
                  ...
                  <h2 className="text-3xl font-black text-rose-400 uppercase tracking-widest mb-2">
                    Eliminated!
                  </h2>
          ...
          <div className="p-3 lg:p-4 bg-[#111827] border-t-2 border-cyan-500 shrink-0 shadow-[0_-8px_30px_rgba(6,182,212,0.15)]">
            ...
            <div className="max-w-3xl mx-auto flex items-center gap-4 relative">
              <span className="absolute left-4 text-cyan-500 font-mono text-xl font-black">
                &gt;
              </span>
              <input
                type="text"
        ...
        <div className="w-full lg:w-72 flex flex-col bg-[#111827] border-l border-cyan-500/20 overflow-y-auto shrink-0">
          <div className="p-4 border-b border-gray-800 bg-[#0a0e1a]">
            <h3 className="text-xs font-bold uppercase tracking-wider text-cyan-400 mb-0.5">
              ⚡ Live Scoreboard
            </h3>
            <p className="text-[10px] text-gray-500">
              Last defender standing wins!
            </p>
          </div>
          <div className="flex flex-col gap-2 p-4">
            {sortedPlayers.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`p-3 rounded-xl border flex flex-col gap-1 transition-all ${isMe ? "bg-cyan-500/10 border-cyan-500/40" : "bg-[#0a0e1a] border-gray-800"} ${p.isEliminated ? "opacity-40 grayscale" : ""}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 font-bold text-xs truncate">
                      <span className="text-xl">{p.character.emoji}</span>
                      <span
                        className={`truncate ${isMe ? "text-cyan-400" : "text-white"}`}
                      >
                        {p.username}
                      </span>
                    </div>
                    <span className="font-mono text-xs text-gray-400 font-bold shrink-0">
                      {p.score}
                    </span>
                  </div>
                  {p.isEliminated && (
                    <div className="text-[9px] uppercase font-black text-rose-500 tracking-widest">
                      💥 Eliminated
                    </div>
                  )}
                </div>
        ...
        <div className="w-full max-w-2xl bg-[#111827] border border-cyan-500/20 rounded-3xl p-8 shadow-[0_0_60px_rgba(6,182,212,0.2)] flex flex-col items-center gap-6 text-center my-8">
          <div className="flex flex-col items-center gap-2">
            ...
            {winner && (
              <p className="text-lg text-cyan-400 font-bold">
                {winner.character.emoji} {winner.username} survived the longest!
              </p>
            )}
          </div>
          ...
          <div className="w-full bg-[#0a0e1a] border border-gray-800 rounded-2xl p-6 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 mt-2">
              {sorted.map((p, i) => (
                <div
                  key={p.id}
                  className={`flex items-center justify-between p-3 rounded-xl border ${i === 0 ? "bg-amber-500/10 border-amber-500/30" : i === 1 ? "bg-slate-800/40 border-slate-700" : "bg-[#111827] border-gray-800/60"}`}
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${i === 0 ? "bg-amber-500 text-white" : i === 1 ? "bg-slate-400 text-white" : "bg-gray-700 text-gray-400"}`}
                    >
                      {i + 1}
                    </span>
          ...
          <div className="flex gap-4 w-full">
            <button
              ...
              className="flex-1 py-3.5 bg-cyan-600 hover:bg-cyan-500 text-white font-black rounded-xl transition shadow-[0_0_15px_rgba(6,182,212,0.3)] text-sm uppercase tracking-wider"
            >
              Play Again
            </button>
            <button
