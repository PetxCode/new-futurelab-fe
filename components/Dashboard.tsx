...
const Dashboard: React.FC<{
  ...
  const [resources, setResources] =
    useState<LearningResource[]>(SUGGESTED_RESOURCES);
  ...
  const handleMissionComplete = async (score: number) => {
    ...
    const updatedRes = updatedResources.find(
      (r) => r.id === selectedResource.id,
    );
    if (updatedRes) setSelectedResource(updatedRes);

    toast.success(
      `${selectedMission.title} marked as done! Next mission unlocked.`,
    );
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/analytics?timeframe=${timeframe}&t=${Date.now()}`,
          {
      ...
      if (userData?.schoolName) {
        try {
          const res = await fetch(
            `${API_BASE_URL}/api/calendar/${encodeURIComponent(userData.schoolName)}`,
            {
              headers: { "x-auth-token": localStorage.getItem("token") || "" },
            },
          );
  ...
  const metrics = [
    {
      label: "Current GPA",
      value: data?.summary?.gpa || "0.00",
      color: "text-indigo-400",
      percentage: 4,
    },
    {
      label: "Intensity Points",
      value: (data?.summary?.techChamp || 0).toString(),
      color: "text-sky-400",
      percentage: 22,
    },
    {
      label: "Lab Hours",
      value: `${data?.summary?.labHours || 0}h`,
      color: "text-rose-400",
      percentage: 12,
    },
    {
  ...
  if (showCodePlayground) {
    return <CodePlayground onBack={() => setShowCodePlayground(false)} />;
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.map((metric, i) => (
          <div
            key={i}
            className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-3xl border border-slate-700/40 hover:border-indigo-500/30 transition-all hover:translate-y-[-4px] group"
          >
            ...
            <div className="mt-4 flex items-baseline space-x-2">
              <span className="text-4xl font-black text-white group-hover:text-indigo-400 transition-colors tracking-tighter">
                {metric.value}
              </span>
              {metric.percentage !== 0 && (
                <span
                  className={`text-[10px] font-black px-1.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400`}
                >
                  +{metric.percentage}%
                </span>
              )}
            ...
            <div className="w-full h-1 bg-slate-700 mt-4 rounded-full overflow-hidden">
              <div
                className={`h-full opacity-60 bg-current ${metric.color}`}
                style={{ width: "70%" }}
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-slate-400 text-sm font-medium">
                Daily study intensity and task completion
              </p>
            </div>
        ...
        <div className="bg-indigo-600 p-8 rounded-[2rem] shadow-2xl shadow-indigo-500/20 relative overflow-hidden flex flex-col">
          ...
          <h3 className="text-xl font-black text-white mb-1 relative">
            Study Quests
          </h3>
          <p className="text-indigo-100 text-xs mb-6 font-bold relative opacity-80 uppercase tracking-widest">
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-black text-white truncate tracking-wider">
                      {res.title}
                    </h4>
                    <p className="text-[10px] text-indigo-100 font-bold uppercase opacity-70">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center space-x-1.5">
                    <div className="px-2 py-0.5 rounded bg-amber-400 text-amber-900 text-[9px] font-black uppercase">
                      {res.reward}
                    </div>
                    <span className="text-[10px] text-indigo-500 bg-white px-2 py-0.5 rounded font-black">
                      {res.estimatedTime}
                    </span>
                  </div>
                  <svg
          ...
          >
            Terms Focus{" "}
            <span className="text-[10px] text-red-500">(Click Here)</span>
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-2xl font-black text-white">
                School Curriculum Calendar
              </h3>
              <p className="text-slate-500 text-sm font-medium">
                Topics covered recently at {userData.schoolName}
              </p>
            </div>
            ...
            {calendarEvents.length > 0 ? (
              calendarEvents.map((event) => {
                ...
                return (
                  <div
                    key={event._id}
                    className="relative bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-[2rem] overflow-hidden hover:border-indigo-500/40 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-500/10 group flex flex-col h-full"
                  >
              ...
              <div className="col-span-full p-8 bg-slate-800/40 border border-slate-700/50 rounded-[2rem] text-center">
                <p className="text-slate-500 font-medium italic">
                  No recent curriculum events posted yet.
                </p>
              </div>
            )}
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium">
              Insights and updates from the FutureLab's content room
            </p>
          </div>
          {(userData?.isAdmin || userData?.isInstructor) && (
            <button
              onClick={() => onNavigate?.("Write Blog")}
